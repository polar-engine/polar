	...
	class asset : public base {
	  ...
	  public:
		...
		void update(DeltaTicks &) override {
			for(auto &pair : partials) {
				...
				if(!partial.done) {
					...
					bool eof   = false;
					partial.contents += fs::local::read(
		...
		std::shared_ptr<T> get(const std::string name, Ts &&... args) {
			...
			if(!partial.asset) {
				debugmanager()->verbose("deserializing asset ",
				                        polar::asset::name<T>(), '/', name);
				...
				deserializer >> *asset;
				partial.asset =
				    std::static_pointer_cast<polar::asset::base>(asset);

				// free up raw asset data
				std::string().swap(partial.contents);

				debugmanager()->verbose("deserialized asset ",
				                        polar::asset::name<T>(), '/', name);
			}
