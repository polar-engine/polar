...
namespace polar::system {
	class action : public system::base {
	  ...
	  public:
		...
		inline auto &current_frame(size_t n = 0) { return framebuffer[framebuffer.size() - 1 - frame_offset - n]; }
		...
		void tick() {
			...
			for(auto &pair : digitals) {
				for(auto &state : pair.second.states) {
					trigger_digital<false>(state.first, pair.first, state.second ? lifetime::when : lifetime::unless);
				}
			}

			for(auto &pair : cf.analogs) {
				for(auto &state : pair.second.states) { trigger_analog(state.first, pair.first); }
			}
			...
			if(frame_offset == 0) {
				...
				for(auto &pair : tmp) {
					...
					if(it == nf.digitals.end()) { nf.digitals.emplace(pair.first, pair.second); }
					// nf.digitals[pair.first] = pair.second;
				}

				for(auto &a : cf.actions) { trigger_digital<false>(a.object, a.ti, a.value); }
				...
				for(auto &pair : tmp2) {
					...
					if(it == nf.analogs.end()) { nf.analogs.emplace(pair.first, pair.second); }
				}
			}
		}
		...
		void reg_digital(core::ref object, std::type_index ti, bool state = false) {
			...
			if(it == cf.digitals.end()) { cf.digitals.emplace(ti, digital_data{}); }
		...
		void reg_analog(core::ref object, std::type_index ti, math::decimal initial = 0) {
			...
			if(it == cf.analogs.end()) { cf.analogs.emplace(ti, analog_data{}); }
			cf.analogs[ti].states.emplace(object, analog_state{initial, initial, initial});
		}

		template<typename T>
		typename std::enable_if<std::is_base_of<digital, T>::value>::type reg_digital(core::ref object,
		                                                                              bool state = false) {
			reg_digital(object, typeid(T), state);
		}

		template<typename T>
		typename std::enable_if<std::is_base_of<analog, T>::value>::type reg_analog(core::ref object,
		                                                                            math::decimal initial = 0) {
			reg_analog(object, typeid(T), initial);
		}

		// digital -> digital function
		template<typename Src, typename = typename std::enable_if<std::is_base_of<digital, Src>::value>::type>
		auto bind(lifetime lt, digital_function_t f, priority_t priority = 0, bool cont = true) {
			...
			return core::ref([this, lt, id] { lt_bindings[size_t(lt)].get<tag_id>().erase(id); });
		}
		...
		template<typename Src, typename Tgt,
		         typename = typename std::enable_if<std::is_base_of<digital, Src>::value>::type,
		         typename = typename std::enable_if<std::is_base_of<digital, Tgt>::value>::type>
		...
		template<typename Src, typename Tgt,
		         typename = typename std::enable_if<std::is_base_of<digital, Src>::value>::type,
		         typename = typename std::enable_if<std::is_base_of<analog, Tgt>::value>::type>
		...
		auto bind(analog_function_t f, priority_t priority = 0,
			...
			auto r             = relation{id, ti, priority, b};
			bindings.insert(r);
			return core::ref([this, id] { bindings.get<tag_id>().erase(id); });
		}
		...
		// analog -> analog
		template<typename Src, typename Tgt,
		         typename = typename std::enable_if<std::is_base_of<analog, Src>::value>::type,
		         typename = typename std::enable_if<std::is_base_of<analog, Tgt>::value>::type>
		...
		template<bool source = true> void trigger_digital(core::ref object, std::type_index ti, lifetime lt) {
			...
			if(lt == lifetime::on) {
				...
				for(auto whenIt = whenPair.first; whenIt != whenPair.second; ++whenIt) {
					...
					if(!whenIt->binding.should_continue(object)) { break; }
				}
			...
			for(auto it = pair.first; it != pair.second; ++it) {
				...
				if(auto wrapper = it->binding.get_if_tgt_digital()) {
					if(lt != lifetime::when) { trigger_digital<false>(object, wrapper->ti); }
				...
				if(!it->binding.should_continue(object)) { break; }
			}
		...
		template<bool source = true> void trigger_digital(core::ref object, std::type_index ti, bool state) {
			auto &cf = current_frame();
		...
		template<bool source = true> void trigger_digital(std::type_index ti, bool state) {
			return trigger_digital<source>(core::ref(), ti, state);
		}

		template<bool source = true> void trigger_digital(core::ref object, std::type_index ti) {
		...
		template<bool source = true> void trigger_analog(core::ref object, std::type_index ti) {
			auto &cf = current_frame();
			...
			for(auto it = pair.first; it != pair.second; ++it) {
				if(auto f = it->binding.get_if_tgt_analog_f()) {
					...
					auto result =
					    it->binding.predicate(object, data.states[object].previous, data.states[object].value);
					trigger_digital<false>(object, wrapper->ti, result);
				}

				if(!it->binding.should_continue(object, data.states[object].value)) { break; }
			}
		}
		...
		void accumulate_analog(core::ref object, std::type_index ti, math::decimal passthrough) {
			...
			for(auto it = pair.first; it != pair.second; ++it) {
				...
				if(!it->binding.should_continue(object, passthrough)) { break; }
			}
		}

		template<typename T, typename = typename std::enable_if<std::is_base_of<digital, T>::value>::type>
		inline void trigger(core::ref object, lifetime lt) {
			trigger_digital(object, typeid(T), lt);
		}

		template<typename T, typename = typename std::enable_if<std::is_base_of<digital, T>::value>::type>
		inline void trigger(core::ref object, bool state) {
			trigger_digital(object, typeid(T), state);
		}

		template<typename T, typename = typename std::enable_if<std::is_base_of<digital, T>::value>::type>
		inline void trigger(bool state) {
			trigger_digital(core::ref(), typeid(T), state);
		}

		template<typename T>
		typename std::enable_if<std::is_base_of<digital, T>::value>::type trigger(core::ref object) {
			trigger_digital(object, typeid(T));
		}

		template<typename T> typename std::enable_if<std::is_base_of<digital, T>::value>::type trigger() {
			trigger_digital(core::ref(), typeid(T));
		}

		template<typename T>
		typename std::enable_if<std::is_base_of<analog, T>::value>::type trigger(core::ref object) {
			trigger_analog(object, typeid(T));
		}

		template<typename T, typename = typename std::enable_if<std::is_base_of<analog, T>::value>::type>
		inline void accumulate(core::ref object, math::decimal x) {
			accumulate_analog(object, typeid(T), x);
		}

		template<typename T, typename = typename std::enable_if<std::is_base_of<analog, T>::value>::type>
		inline void accumulate(math::decimal x) {
			accumulate_analog(core::ref(), typeid(T), x);
		}
	};
} // namespace polar::system
