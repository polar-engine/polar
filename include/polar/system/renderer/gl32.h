	...
	namespace renderer {
		class gl32 : public base {
		  ...
		  private:
			...
			inline std::shared_ptr<model_p>
			getpooledmodelproperty(const GLsizei required) {
				if(modelPropertyPool.empty()) {
					...
					GL(glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0,
					                         NULL));

					GL(glBindBuffer(GL_ARRAY_BUFFER, prop.vbos[1]));
					GL(glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0,
					                         NULL));
			...
			inline void uploadmodel(std::shared_ptr<component::model> model) {
				...
				auto dataSize =
				    sizeof(component::model::PointType) * numVertices;
			...
			inline void
			componentadded(IDType id, const std::type_info *ti,
				...
				if(ti == &typeid(component::model)) {
					...
					auto sprite =
					    std::static_pointer_cast<component::sprite::base>(
					        ptr.lock());
					sprite->render();
					sprite_p prop;
					...
					GL(glTexImage2D(GL_TEXTURE_2D, 0, format,
					                sprite->surface->w, sprite->surface->h, 0,
					...
					auto &cache = fontCache.emplace(text->as, fontcache_t())
					                  .first->second;
					...
					for(auto c : text->str) {
						...
						if(!entry.active) {
							...
							GLint format = GL_RGBA;
							GL(glTexImage2D(GL_TEXTURE_2D, 0, format,
							                glyph.surface->w, glyph.surface->h,
		  ...
		  public:
			...
			void
			setpipeline(const std::vector<std::string> &names) override final {
				pipelineNames = names;
				if(inited) {
