...
namespace polar::system {
	class ttl : public base {
	  ...
	  public:
		...
		void component_added(core::weak_ref wr, std::type_index ti, std::weak_ptr<component::base>) override {
			if(ti == typeid(component::ttl)) {
				objects.emplace(wr.own());
			}
		}

		void component_removed(core::weak_ref wr, std::type_index ti, std::weak_ptr<component::base>) override {
			if(ti == typeid(component::ttl)) {
