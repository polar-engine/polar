...
namespace polar { namespace system {
	...
	public:
		...
		struct tween_desc {
			...
			tween_desc(T from, T to, double in, double pause, bool loop, tween_handler fn, double acc)
	...
	public:
		...
		inline std::shared_ptr<core::destructor> tween(T from, T to, double in, bool loop, tween_handler fn, double pause, T initial) {
			auto id = nextID++;
