...
namespace polar { namespace system {
	...
	public:
		...
		inline std::shared_ptr<core::destructor> listen(const global_listener_t &fn) {
			auto id = nextID++;
		...
		inline std::shared_ptr<core::destructor> listenfor(const std::string &msg, const listener_t &fn) {
			return listenfor("", msg, fn);
		}

		inline std::shared_ptr<core::destructor> listenfor(const std::string &ns, const std::string &msg, const listener_t &fn) {
			auto m = ns + '.' + msg;
			auto id = nextID++;
		...
		inline void firein(const std::string &ns, const std::string &msg, arg_t arg = nullptr) const {
			auto m = ns + '.' + msg;
