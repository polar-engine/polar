...
namespace polar { namespace support { namespace ui {
	...
	public:
		...
		menuitem(std::string value, std::function<FTy> fn) : menuitem(value, control::button(), fn) {}
		template<typename T> menuitem(std::string value, T control, std::function<FTy> fn) : value(value), fn(fn) {
			static_assert(std::is_base_of<control::base, T>::value, "menuitem requires object of base class polar::support::ui::control::base");
