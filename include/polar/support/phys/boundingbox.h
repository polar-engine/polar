	...
	namespace phys {
		struct boundingbox {
			...
			TestRay(const Point3 &origin, const Point3 &direction, Decimal tMax,
				...
				auto failure = std::make_tuple(
				    false, std::numeric_limits<Decimal>::infinity(),
			...
			AABBSwept(const boundingbox &b, const Point3 &ownPos,
				...
				if(!AABBCheck(broadphase, ownPos, b.position)) {
					return result;
				}
				...
				if(entryTime == entry.x) {
					std::get<1>(result) = Point3(
					    inverseEntry.x < 0.0f ? 1.0f : -1.0f, 0.0f, 0.0f);
				} else if(entryTime == entry.y) {
					std::get<1>(result) = Point3(
					    0.0f, inverseEntry.y < 0.0f ? 1.0f : -1.0f, 0.0f);
				} else {
					std::get<1>(result) = Point3(
					    0.0f, 0.0f, inverseEntry.z < 0.0f ? 1.0f : -1.0f);
				}
