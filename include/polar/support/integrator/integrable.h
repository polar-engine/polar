...
namespace polar { namespace support { namespace integrator {
	...
	public:
		...
		template<typename _To> inline _To to_previous() { return static_cast<_To>(previousValue); }
		...
		inline integrable_base & getderivative(const unsigned char n = 0) override final {
			if(n == 0) {
