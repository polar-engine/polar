...
namespace polar { namespace support { namespace work {
	...
	public:
		...
		job(const job_fn &&fn, const job_priority &&priority = job_priority::normal, const job_thread &&thread = job_thread::any)
			: type(job_type::work), priority(priority), thread(thread), fn(fn) {}
		job(const job_type &&type, const job_priority &&priority = job_priority::normal, const job_thread &&thread = job_thread::any)
