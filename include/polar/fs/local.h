	...
	class local {
	  ...
	  public:
		...
		static std::string savedgamesdir(std::string name) {
			...
			auto result = SHGetFolderPathA(
			    NULL, CSIDL_PERSONAL | CSIDL_FLAG_CREATE, NULL, 0, szPath);
			if(FAILED(result)) {
				debugmanager()->fatal(
				    "CSIDL_PERSONAL: failed to retrieve path");
			}
		...
		static std::vector<std::string> listdir(std::string path) {
			...
			while((ep = readdir(dp))) {
				...
				if(result < 0) {
					debugmanager()->fatal(path, ": fstatat failed on ",
					                      ep->d_name);
				}
