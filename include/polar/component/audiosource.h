...
namespace polar { namespace component {
	...
	public:
		...
		audiosource(std::shared_ptr<asset::audio> as, sourcetype type, const bool looping = false)            : asset(as), type(type), looping(looping), loopIn(0),        loopOut(as->samples.size()) {}
		audiosource(std::shared_ptr<asset::audio> as, sourcetype type, const loopin  &in)                     : asset(as), type(type), looping(true),    loopIn(in.value), loopOut(as->samples.size()) {}
		audiosource(std::shared_ptr<asset::audio> as, sourcetype type, const loopout &out)                    : asset(as), type(type), looping(true),    loopIn(0),        loopOut(out.value) {}
		audiosource(std::shared_ptr<asset::audio> as, sourcetype type, const loopin  &in, const loopout &out) : asset(as), type(type), looping(true),    loopIn(in.value), loopOut(out.value) {}
