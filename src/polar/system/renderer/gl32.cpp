...
namespace polar { namespace system { namespace renderer {
	...
	void gl32::update(DeltaTicks &dt) {
		...
		for(auto itRight = pairRight.first; itRight != pairRight.second; ++itRight) {
			...
			if(pos != nullptr) { cameraView = glm::translate(cameraView, -pos->pos.temporal(alpha).to<Point3>()); }
		}
	...
	void gl32::rendersprite(IDType id) {
		...
		if(scale) {
			transform = glm::scale(transform, scale->sc.get());
		}

		uploaduniform(spriteProgram, "u_color", color ? color->col.get() : Point4(1));
	...
	void gl32::rendertext(IDType id) {
		...
		for(auto c : text->str) {
			...
			glyphTransform = glm::translate(glyphTransform, Point3(text->as->glyphs[c].min.x, 0, 0));

			// scale to glyph size
			auto sc = Point3(text->as->glyphs[c].surface->w, text->as->glyphs[c].surface->h, 1);
